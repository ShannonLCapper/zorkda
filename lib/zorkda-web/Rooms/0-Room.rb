class Room
	...
	:has_entry_cutscene, :checkpoint_cutscene_list, :floor, :navi_hint
	...
	def update_room_conditions(game_status)
		...
		update_bombs(game_status)
		update_switches(self.inventory, move_counter)
		update_temp_torches(self.inventory, move_counter)
		update_locks(game_status)
		update_distances(game_status)
		check_respawn_while_in_room(player, move_counter)
		update_names
		update_navi_hint(game_status)
	...
	def update_bombs(game_status)
		...
		self.inventory.each do |item|
			if (item.is_a?(Bomb_flower) || item.is_a?(Lit_bomb)) && item.fuse_lit 
				if move_counter > item.moves_when_activated + item.moves_to_explode
					puts "The #{item.name} explodes."
					item.fuse_lit = false
	...
	def move_all_closer
		...
		if things_now_in_range.length > 0
			print_array(things_now_in_range, "The following are now within reach:")
		end
		if sides_now_in_range.length > 0
			print_array(sides_now_in_range, "You can now reach the following directions:")
		end
	end
	...
	def move_all_farther_if_needed
		...
		if things_no_longer_in_range.length > 0
			print_array(things_no_longer_in_range, "These are now out of reach:")
		end
		if sides_no_longer_in_range.length > 0
			print_array(sides_no_longer_in_range, "You can no longer go")
			puts "\b."
		end
	end
