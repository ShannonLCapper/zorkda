module Zorkda
	module Engine
		...
		#DONE
		def self.describe_directions_in_room(room)	
			sides = []
			...
			similar_side_groups.each do |similar_sides|
				if similar_sides.length == 1
					...
					text_line = "To the #{directions.first}"
					if directions.length > 2
						text_line += ","
						directions[1...-1].each { |direction| text_line += " #{direction}," }
					end
					text_line += " and #{directions.last}, #{similar_sides.first.plural_description}."
