def break(game_status, target, with_obj)
	player = game_status.player
	curr_room = game_status.curr_room
	move_counter = game_status.move_counter
	with_obj = equate_with_obj_to_obj_player_has(player, with_obj, "break")
	if with_obj == nil
		return
	elsif !with_obj.can_hit_things
		puts "That's not something you can break things with."
		return
	end
	...
	if matches.length == 0
		...
		puts "You're going to have to be more specific about what you want to break."
		list_options(matches)
		puts "Retype the \"break\" command with the target you want."
		return
	else
		target = matches[0]
		if target.distance > with_obj.range
			puts "Your target is too far away."
		elsif !target.breakable
			puts "That's not something you can break."
		else
