def initialize_new_game(file_name)
	#create Link
	player = Protagonist.new("Link")
	...
	#name link
	unless file_name == "File_1"
		name_confirmed = "no"
		until name_confirmed == "yes"
			puts "\nWhat would you like to name your character?"
			player_name = prompt
			puts "Are you happy with the name \"#{player_name}\"?"
			name_confirmed = get_valid_yes_or_no
		end
	...
	game_status = Game_status.new(file_name, player, child_rooms[:links_house], child_rooms)
	save(game_status)
