module Zorkda
  module Actors		
  	...
  	#DONE
		class KfCentralKokiri < Character
			...
			def dialogue(game_status)
				if self.talked_to_checkpoints.length == 0 && game_status.checkpoint < 2
					...
					return [
						"Oh, you have a fairy now?! That's great, #{game_status.player.name}!",
						"What? You've been called by the Great Deku Tree? What an honor! " +
					...
					return "I wonder if the Great Deku Tree gave life to everying in the forest, I mean in addition to us Kokiri?"
				end
			end
		end
